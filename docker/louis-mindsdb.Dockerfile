...
RUN --mount=type=cache,target=/root/.cache \
    uv venv /venv \
    && uv pip install --index-url https://pypi.tuna.tsinghua.edu.cn/simple  pip "." --prerelease=allow
...
RUN --mount=type=cache,target=/root/.cache \
    if [ -n "$EXTRAS" ]; then uv pip install --index-url https://pypi.tuna.tsinghua.edu.cn/simple $EXTRAS; fi
...
RUN --mount=type=cache,target=/root/.cache uv pip install --index-url https://pypi.tuna.tsinghua.edu.cn/simple -r requirements/requirements-dev.txt \
                                        && uv pip install --index-url https://pypi.tuna.tsinghua.edu.cn/simple --no-deps -e "."
